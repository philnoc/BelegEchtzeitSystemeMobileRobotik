...
class Routine(Node):
    def __init__(self):
        ...
        self.declare_parameter('backup_time', 2.0)        # s
        self.declare_parameter('max_forward_time', 30.0)  # s failsafe
    ...
    def switch(self, new_phase: Phase):
        ...
        if new_phase == Phase.DRIVE_TILL_HAZARD:
            ...
            # nichts; wir fahren einfach forward_elapsed Sekunden vorwärts
            pass
    ...
    def step(self):
        ...
        if self.phase == Phase.DRIVE_TILL_HAZARD:
            ...
            if self.elapsed_phase() <= max(0.0, self.forward_elapsed):
                cmd = Twist(); cmd.linear.x = v_fwd
                self.cmd_pub.publish(cmd)
            else:
